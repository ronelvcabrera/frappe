...
frappe.ui.form.Timeline = class Timeline {
  ...
  make() {
    var me = this;
    this.wrapper = $(
      frappe.render_template("timeline", {
        ...
        callback: function(r) {
          if (!r.exc) {
            if (r.message) {
              ...
              var communications = me
                .get_communications()
                .concat(new_communications);
  ...
  display_automatic_link_email() {
    ...
    if (docinfo.document_email) {
      let link = __("Send an email to {0} to link it here", [
        `<b><a class="timeline-email-import-link copy-to-clipboard">${docinfo.document_email}</a></b>`
      ]);
  ...
  setup_interaction_button() {
    ...
    this.activity_button = this.wrapper.find(selector).on("click", function() {
      var args = {
        doc: me.frm.doc,
        frm: me.frm,
        recipients: me.get_recipient()
  ...
  refresh(scroll_to_end) {
    ...
    this.wrapper
      .find(".is-email")
      .prop("checked", this.last_type === "Email")
      .change();
  ...
  render_timeline_item(c) {
    ...
    var $timeline_item = $(
      frappe.render_template("timeline_item", { data: c, frm: this.frm })
    )
      .appendTo(me.list)
      ...
      .on("click", ".edit-comment", function(e) {
        ...
        if ($timeline_item.hasClass("is-editing")) {
          ...
          const $edit_btn = $(this);
          const $timeline_content = $timeline_item.find(
  ...
  prepare_timeline_item(c) {
    ...
    if (
      !c.doctype &&
      ["Comment", "Communication"].includes(c.communication_type)
    ) {
    ...
    if (
      ...
      (c.comment_type || "Comment") === "Comment"
    ) {
      if (frappe.user.name == c.sender && frappe.model.can_delete("Comment")) {
        c["delete"] =
          '<a class="close delete-comment" title="Delete"  href="#"><i class="octicon octicon-x"></i></a>';
      }

      if (frappe.user.name == c.sender || frappe.user.name == "Administrator") {
        c["edit"] = `<a class="edit-comment text-muted" title="${__(
          "Edit"
        )}" href="#">${__("Edit")}</a>`;
      }
    }
    ...
    c.futur_date =
      communication_date > frappe.datetime.now_datetime() ? true : false;
    if (!c.fullname) {
    ...
    if (c.comment_type === "Workflow" || c.comment_type === "Label") {
      c.comment_html = repl(
        '<span class="label label-%(style)s">%(text)s</span>',
        {
  ...
  is_communication_or_comment(c) {
    return (
      ...
      (c.communication_type === "Comment" &&
        (c.comment_type === "Comment" || c.comment_type === "Relinked"))
    );
  }
  ...
  build_version_comments(docinfo, out) {
    ...
    docinfo.versions.forEach(function(version) {
      ...
      if (data.comment) {
        out.push(
          me.get_version_comment(version, data.comment, data.comment_type)
        );
        return;
      }
      ...
      if (data.changed && data.changed.length) {
        ...
        if (parts.length) {
          let message;
          if (data.data_import) {
            message = __("changed value of {0} {1}", [
              parts.join(", ").bold(),
              data_import_link
            ]);
          } else {
      ...
      if (data.row_changed && data.row_changed.length) {
        ...
        if (parts.length) {
          let message;
          if (data.data_import) {
            message = __("changed values for {0} {1}", [
              parts.join(", "),
              data_import_link
            ]);
          } else {
      ...
      ["added", "removed"].forEach(function(key) {
        if (data[key] && data[key].length) {
          ...
          if (parts.length) {
            out.push(
              me.get_version_comment(
                version,
                __("{0} rows for {1}", [__(key), parts.join(", ")])
              )
            );
          }
        }
      ...
      return frappe.call({
        ...
        callback: function(r) {
          if (!r.exc) {
            ...
            me.frm.get_docinfo().communications = $.map(
  ...
  get_names_for_mentions() {
    var valid_users = Object.keys(frappe.boot.user_info).filter(
  ...
  setup_comment_like() {
    this.wrapper.on(
      "click",
      ".comment-likes .octicon-heart",
      frappe.ui.click_toggle_like
    );
...
$.extend(frappe.timeline, {
  new_communication: function(communication) {
    ...
    var docinfo = frappe.model.get_docinfo(
    ...
    if (
      cur_frm.doctype === communication.reference_doctype &&
      cur_frm.docname === communication.reference_name
    ) {
